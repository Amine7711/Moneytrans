...
try:
    data = pd.read_csv(r"C:\Users\amine\Downloads\liquidite_banque_maroc_2023_2025_utf8.csv")
...
if st.button("🔍 تحليل البيانات"):
    ...
    if filtered.empty:
        st.warning("⚠ لا توجد بيانات متوفرة ضمن هذا النطاق الزمني.")
        st.stop()
    ...
    st.subheader("⚠ مراقبة السيولة المنخفضة")
    ...
    if avg_trend > 0 and ma_trend > 0:
        ...
        st.info("⚖ السيولة تعرف تقلبات في الأيام الأخيرة.")
    ...
    if volatility > 15:
        st.warning("⚠ تغيرات كبيرة تشير إلى عدم استقرار.")
    elif volatility > 5:
